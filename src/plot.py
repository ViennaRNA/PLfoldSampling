...
def plot_unpaired(dataset=None,
                  figure_path=None,
...
def heatmap(dataset,
    ...
    if what=='joint_paired':
        cond_unpaired_P = copy.deepcopy(dataset.local_sampling.joint_paired_P[start:end, start:end])
        unpaired_P = np.array(copy.deepcopy(dataset.local_sampling.paired_P[start:end]))
    elif what=='joint_unpaired':
        cond_unpaired_P = copy.deepcopy(dataset.local_sampling.joint_unpaired_P[start:end, start:end])
        unpaired_P = np.array(copy.deepcopy(dataset.local_sampling.unpaired_P[start:end]))
    elif what=='cond_paired':
        cond_unpaired_P = copy.deepcopy(dataset.local_sampling.cond_paired_P[start:end, start:end])
        unpaired_P = np.array(copy.deepcopy(dataset.local_sampling.paired_P[start:end]))
    elif what=='cond_unpaired':
        cond_unpaired_P = copy.deepcopy(dataset.local_sampling.cond_unpaired_P[start:end, start:end])
        unpaired_P = np.array(copy.deepcopy(dataset.local_sampling.unpaired_P[start:end]))
    elif what=='rnafold_bpp':
        cond_unpaired_P = copy.deepcopy(dataset.global_folding.bp_P[start:end, start:end])  
        unpaired_P = np.array(copy.deepcopy(dataset.local_sampling.paired_P[start:end]))
    ...
    gs = GridSpec(2, 2, left=0, right=1, wspace=-0.3, hspace=0.08, width_ratios=[1, len(unpaired_P)], height_ratios=[len(unpaired_P), 1])
...
def struct_ps(
    ...
    if context:
        ...
        bp_list = [ pair for pair in bp_list if pair[0]>=start-1 and pair[1] <= end-1]
    ...
    if annotations:
        ...
        for sub_s in annotations:
            annotation_string += f'{sub_s[0]+1-new_start} {sub_s[1]+1-new_start} 12 0.8 0.8 0.8 omark '
...
def difference(dataset, grey=False):
    ...
    data = []
    for d in dataset:
        data.append({'win_len': d.window_size,
    ...
    df['diff_sum'] = df.apply(lambda row : diff_sum(row['up_sample'], row['up_plfold']), axis = 1)
    df['diff_norm'] = df.apply(lambda row : diff_norm(row['up_sample'], row['up_plfold']), axis = 1)
    df['diff_rel'] = df.apply(lambda row : diff_rel(row['up_sample'], row['up_plfold']), axis = 1)
    df['diff_per_nt'] = df.apply(lambda row : diff_per_nt(row['up_sample'], row['up_plfold']), axis = 1)

    return df

def plot_diff_per_samplesize(id,
                            df,
                            diff,
                            figure_path=None,
                            grey=False):
    ...
    if grey == False:
        colors = ['tab:blue','tab:orange','tab:green','tab:red','tab:purpel',
                    'tab:brown','tab:pink','tab:gray','tab:olive','tab:cyan']
        for i, g in enumerate(groups):
            g[1].plot.scatter(x = 'samplesize',
                        y =diff,
                        ax = ax,
                        legend = True,
                        label = g[0],
...
def plot_diff_per_samplesize_window(id,
                                    df,
                                    diff,
                                    figure_path=None,
                                    grey=False):
...
def plot_unpaired_hist(datasets,
                        figure_path = None,
                        tsv_file = None):
    ...
    for i, dataset in enumerate(datasets):
        ...
        data[str(windowsize).zfill(4) + ' RNAfold'] = dataset.global_folding.unpaired_P[1:]#[0]
        data[str(windowsize).zfill(4) + ' RNAPLfold'] = dataset.local_sampling.unpaired_P

    df = pd.DataFrame.from_dict(data)
    df = df.reindex(sorted(df.columns), axis=1)
    fig, axs = plt.subplots(figsize=(6.4*cm, 8*cm),
    ...
    plot_columns = plot_columns + [[str(c) for c in df.columns if str('RNAfold') in str(c)][0]]
    for i, c in enumerate (plot_columns):
        bins = [b/20 for b in list(range(0,21))]
        df.hist(c, ax = axs[i], bins = bins, grid = False, color = 'lightgrey')#legend = str(c))
...
def plot_error_vs_windowsize_boxplot(datasets,
                                     figure_path=None,
                                     tsv_file =None):
    ...
    for dataset in datasets:
        ...
        data[str(windowsize).zfill(4) + ' absdiff'] = np.absolute(data[str(windowsize).zfill(4) + ' diff'])
    ...
    df.boxplot(plot_columns, ax = axs, whis = (5, 95), showfliers = False, grid = False,
               whiskerprops=whiskerprops,
               boxprops = boxprops,
               medianprops = medianprops,
              )
    axs.set_ylim(-0.1,1.1)
    axs.set_xticklabels([l.get_text().split(' ')[0].lstrip('0') for l in axs.get_xticklabels()],rotation = 90)
...
def plot_scatter_unpaired(dataset,
                          figure_path = None,
                          tsv_file = None
                          ):
    ...
    spec = fig.add_gridspec(ncols=3, nrows=2, width_ratios=widths, height_ratios=heights,
                            wspace=0.0001, hspace=0.0001)  # setting spaces
    ...
    # 3.2. scatterplot
    df.plot.scatter(x = str(windowsize1).zfill(4) + ' RNAPLfold' , y = str(windowsize1).zfill(4) + ' RNAfold', alpha = 0.4, ax = axs[3], s=0.5,  c = '#333333')
    axs[3].set_ylim(0,1)
    axs[3].set_xlim(0,1)

    # 3.3. histogram (bill_length_mm)
    df.hist(str(windowsize1).zfill(4) + ' RNAPLfold', ax = axs[0], bins = bins, grid = False, color = 'grey')
    ...
    df.hist(str(windowsize1).zfill(4) + ' RNAfold', ax = axs[5], bins = bins, grid = False, color = 'grey', orientation='horizontal')
    axs[5].set_ylim(0,1)
    axs[5].set_ylabel('')
    ...
    # 4.2. scatterplot
    df.plot.scatter(x = str(windowsize2).zfill(4) + ' RNAPLfold' , y = str(windowsize2).zfill(4) + ' RNAfold', alpha = 0.4, ax = axs[4], s=0.5, c = '#333333')
    axs[4].set_ylim(0,1)
    axs[4].set_xlim(0,1)

    # 4.3. histogram (flipper_length_mm)
    df.hist(str(windowsize2).zfill(4) + ' RNAPLfold', ax = axs[1], bins = bins, grid = False, color = 'grey')
...
def plot_bp_span_hist(dataset,
    ...
    for d in dataset:
        ...
        df1.rename(columns={'value':'rnafold '+ str(d.window_size) + ' ' + str(d.max_bp_span)}, inplace = True)
        df2 = get_dist_df(d.local_sampling.bp_P)
        df2.rename(columns={'value':'stochastic '+ str(d.window_size) + ' ' + str(d.max_bp_span)}, inplace = True)
    ...
    fig.suptitle('frequency of base pair spans\nsequence length: {seq_len}; sample size: {sample_size}'.format(seq_len = len(dataset[0].sequence), sample_size = dataset[0].num_samples))
    if figure_path is not None:
        fig.savefig(figure_path, bbox_inches="tight")
